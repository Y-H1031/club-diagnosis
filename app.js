...
// 設定（GASのウェブアプリURL）
// =========================
const GAS_URL = "https://script.google.com/macros/s/AKfycbyZxBwyN-f0O1zsVGwtfNwP9stsTqdc9TmJ_domGQqTT96CAMpuuiO4buUbYqb4wBf0Rw/exec";
...
async function calculate() {
  ...
  // ★万が一エラーが起きてもボタンがフリーズしないように全体を保護します
  try {
    ...
    // ここで項目名（labels）を1回だけ定義します
    const labels = ['男女比', '活動場所', '活動頻度', '雰囲気', '人間関係', '費用'];
    const priorityLabels = Array.from(checkedBoxes).map(box => labels[Number(box.value)]);
    ...
    // スプレッドシートに送るデータの作成（重視項目1・2もここで入れます）
    const sendData = {
      type: selectedType,
      ans_gender: user[0],
      ans_place: user[1],
      ...
      ans_rel: user[4],
      ans_cost: user[5],
      priority1: priorityLabels[0] || "", 
      priority2: priorityLabels[1] || ""  
    };
    for (let i = 0; i < 10; i++) {
    ...
    if (results.length === 0) {
      ...
      resultDiv.innerHTML = results.map((r, i) => {
        ...
        let snsHtml = `<div style="margin: 10px 0; display: flex; gap: 8px; justify-content: center;">`;
        if (r.instagram) {
          snsHtml += `<a href="${r.instagram}" target="_blank" style="text-decoration:none; background: linear-gradient(45deg, #f09433 0%, #e6683c 25%, #dc2743 50%, #cc2366 75%, #bc1888 100%); color: white; padding: 5px 12px; border-radius: 15px; font-size: 0.8em; font-weight: bold;">Instagram</a>`;
        }
        if (r.x_link) {
          snsHtml += `<a href="${r.x_link}" target="_blank" style="text-decoration:none; background: #000; color: white; padding: 5px 12px; border-radius: 15px; font-size: 0.8em; font-weight: bold;">X (Twitter)</a>`;
        }
        snsHtml += `</div>`;
      ...
      results.forEach((r, i) => {
        ...
        const chart = new Chart(ctx, {
          ...
          data: {
            labels: labels,
            datasets: [
              { label: 'あなたの希望', data: user, backgroundColor: 'rgba(0, 123, 255, 0.2)', borderColor: 'rgba(0, 123, 255, 1)', borderWidth: 2 },
              { label: r.name + ' の実態', data: r.data, backgroundColor: 'rgba(255, 99, 132, 0.2)', borderColor: 'rgba(255, 99, 132, 1)', borderWidth: 2 }
            ]
          },
    ...
    // エラーが起きた場合はここでキャッチし、フリーズを防ぐ
    console.error("エラーが発生しました:", error);
    alert("エラーが発生しました。ブラウザを更新してもう一度お試しください。");
  } finally {
    // 正常でもエラーでも、必ずボタンを元に戻す
    btn.disabled = false;
